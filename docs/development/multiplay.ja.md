...
セキュリティを考慮すると、知人、知り合いなどと勝負する場合は、その場限りの使い捨てのテザリングの名前、パスワードを設定し、後で変えておくのがベストかと思います。
...
起動引数の正本は `aichallenge/simulator_scripts/multiplay-host.sh` / `multiplay-client.sh` です。
...
host / client それぞれのスクリプトに1行追加します。

```bash
    --multiplay-name descto \  # ← 好きな名前（最大6文字）
```

全員が接地するとカウントダウンが始まり、レース開始です。
