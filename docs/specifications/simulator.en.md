...
- The practice modes have unlimited laps and timeout (`e2e` uses `--timeout 10000000.0`; `dev` additionally uses `--laps unlimited`), so the car keeps running without stopping. If you want to check with a time limit, use a finals mode or `make eval`.
