...
AWSIMは各車両のドメイン（N=1〜4）に直接接続し、各ドメインでトピックをPublishします。
...
自動走行プログラムからブーストを発動するには、`/awsim/cmd`（`std_msgs/msg/Float32MultiArray`）のインデックス0を `1.0` 以上に立ち上げます。再度発動するには、一度 `0.0` に戻してから再度 `1.0` に立ち上げてください。
