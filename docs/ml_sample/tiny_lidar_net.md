...
学習用データとして、LiDARを有効にした状態で走行したrosbagを保存します。走行は手動走行が望ましいですが、MPCなどの制御アルゴリズムで走行させたデータを使用することもできます。
...
??? note "1. rosbagを訓練データと検証データに割り振る"
    ここでは、流れを掴むために訓練データと検証データに取得したデータをそのままコピーしています。実際には、訓練データと検証データを別のものにする、データを選定するなどの工夫を行ってください。
