...
2. driver・zenoh（通信ミドルウェア） が起動済み（rosbag は運営が記録します。参加チームへの提供はありません）
    - 期日までにソースコードを提出したチームのみ
3. ブースの PC から車両 PC へ SSH 接続済み

参加チームはこの状態から操作を引き継ぎます。車両 PC 上の操作はすべてこの SSH 接続経由で行います。**Autowareの起動、パラメータ調整、必要に応じたモード切替と手動走行、が参加者の責務となります。**
...
| 番号・項目 | 用途 | 当日の扱い |
| --- | --- | --- |
| 1 `Update the acces/brake maps and IMU bias` | 運営が調整したパラメータで上書きする | Autoware起動前に実行します（必須ではないが推奨） |
| 2 `autoware-vehicle` | Autoware の起動 | 再起動時も同様です |
| 3 `check autoware` | 起動後の環境確認 | Autowareを起動した際には実行します。NGが無いことを確認します。 |
| 4 `autoware-vehicle down` | Autoware の停止 | 再起動時に使います |

Autoware を起動する場合は、次の順で操作します。

1. `1 Update the acces/brake maps and IMU bias` を選んで Enter を押し、 `OK` と表示されることを確認します。（オプショナル）
2. `2 autoware-vehicle` を選んで Enter を押し、 `OK` と表示されることを確認します。
3. `3 check autoware` を選んで Enter を押し、`OK` と表示されることを確認します。`NG` の場合は走行を開始せず、表示された失敗内容を運営に伝えます。
4. RViz で自己位置や車両の状態を確認してから走行を再開します。最初は「ステアのみ自動」で低速走行で挙動を確認することを推奨します。

Autoware を再起動する場合は、次の順で操作します。

1. 運営に再起動することを申告し、車両を安全に停止させます。
2. `4 autoware-vehicle down` を選んで Enter を押し、 `OK` と表示されることを確認します。
3. 上述の起動の手順で、Autowareを起動します

!!! note "check autowareのNG原因"

    車両の位置が想定と異なり、計算が収束せずにcontrol_cmdが出力されないことがあります。このような場合には参加者側コードの問題となります。パラメータ調整で対応するか、手動走行に切り替えて車両の位置を調整してください。
...
走行の様子は RViz でも確認できます（[遠隔操作](remote.md)）。
