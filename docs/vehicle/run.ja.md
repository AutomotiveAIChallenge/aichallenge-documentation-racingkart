...
走行枠中の操作は、`make vehicle-tui` で起動する車両コンソールに集約されています。tmux 上で動く TUI で、事前チェック・提出物のダウンロード・ビルド・ドライバ起動・終了処理を1画面から順に実行できます。内部で呼んでいるのは以下の make ターゲットと `setup_check.sh` なので、個別に実行しても構いません。
