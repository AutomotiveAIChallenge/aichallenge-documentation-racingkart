...
syslog転送機能を持つルーターを使う場合はこの設定をすることで、回線が不安定になったときにこのログを活用することができます。
...
反映と待ち受けを確認し、ファイアウォールの許可とホームからの参照用リンクを作ります（`ufw allow` の行は、4-2 の手順でファイアウォールを無効化したままの場合は今すぐには効果がありませんが、後で `ufw enable` して有効に戻したときにこのルールが適用されるよう、いまのうちに追加しておいてください）。
