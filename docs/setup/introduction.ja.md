...
- `./setup.bash doctor` コマンドで環境チェックを実行できます。警告（warning）や情報（info）が表示された場合は、それぞれに示された対策をお試しください
- NVIDIA 関連のエラーが出ていた場合・AWSIM が起動しない場合・描画に問題がある場合は、[GPU の設定・動作設定](./gpu-simulation.ja.md)をご確認ください
    - AI 部門に参加する場合は Camera/LiDAR を有効にする必要があります。同ページの手順をご確認ください
- 稀にAWSIMの起動に失敗することがあります。AWSIMの画面のみ黒画となってしまう場合は、 `make down_all` コマンドで全コンテナを終了してから、再度試してください
