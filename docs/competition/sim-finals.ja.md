...
| 13:00 | 13:10 | Sim to Real部門 学生クラス 第1試合 |
| 13:10 | 13:30 | Sim to Real部門 学生クラス 第2試合 |
| 13:30 | 13:50 | Sim to Real部門 学生クラス 第3試合 |
| 13:50 | 14:10 | Sim to Real部門 学生クラス 第4試合 |

- ※試合開始40分前に、会場に用意したリハーサルテーブルにお集まりください。その後、試合開始20分前に運営案内によって待機場所に移動していただきます。遅れた場合は棄権・不戦敗扱いとなる可能性があります。
- ※学生クラス 第1試合のみセットアップは12:50から行います。そのため、12:10にリハーサルテーブルにお集まりください。
- ※各試合後に10分間の走行振り返りを行います。そのため、一般クラス 第4試合の終了は12:10、学生クラス 第4試合の終了は14:30になります。
...
- ※試合開始40分前に、会場に用意した待機場所にお集まりください。遅れた場合は棄権・不戦敗扱いとなる可能性があります。
- ※決勝戦については、16:30に待機場所にお集まりください。対象は選抜戦 第4試合への参加チーム以外の全チームです。第4試合が終了次第、決勝戦進出チームの発表を行います。待機場所にいない場合には棄権・不戦敗扱いとなる可能性があります。
...
- 説明員：本番走行中に司会者からの質問への受け答えを行う。本番走行後の振り返りを行う（オペレータと兼務可）
...
- セットアップでは、(ソースコードのダウンロード)、ビルド、Autowareの起動を行います。持参したPCを使用する場合にはネットワークの設定も必要になります。
    - 前日（9/18）18時の最新ソースコードを運営側で事前にダウンロード予定です。時間短縮・予期せぬネットワークトラブル防止のため、出来るだけ事前にダウンロード済みのファイルをご使用ください。
...
- オペレーター：Autoware PCのセットアップ・操作を行う
- 説明員：本番走行前に5分間のプレゼンテーション発表を行う。本番走行中に司会者からの質問への受け答えを行う
...
- 1回の試合は上記タイムテーブルに沿って行われます。
- 開始時刻の40分前までに、メイン会場前方の待機場所にお集まりください。
...
- セットアップと並行して、各チーム5分でプレゼンを行っていただきます。
    ...
    - 事前に発表用資料の送付をお願いいたします。

### 試合の流れ（決勝戦） { #e2e-flow-final }
...
- 開始時刻の40分前までに、メイン会場前方の待機場所にお集まりください。
    - 対象は選抜戦 第4試合への参加チーム以外の全チームです
- 決勝戦は上記タイムテーブルに沿って行われます。基本的な流れは選抜戦と同じですが、プレゼンテーションがありません。
