...
- **ソースコードの配置**：`~/aichallenge-racingkart` に、最新の main ブランチの AI チャレンジリポジトリが配置されています。ただし、誤操作防止等のため、一部ファイルは変更されています。
...
1. **ソースコードをダウンロードする**（`make download`）
    ...
    - 注意：万が一ダウンロードに時間がかかりすぎる場合は、運営に申告してください。運営が事前にダウンロードした提出済みソースコードをお渡しします。
2. **ソースコードを確認する**（`code .`）
    - VSCode が開くので、ソースコードが **自チームのもの** であることを確認してください。
    - 確認後は、負荷低減のため VSCode を閉じることを推奨します。
3. **ビルドする**（`make autoware-build`）
    - 注意：ルールに記載の通り、万が一時間内にビルド等のセットアップが完了しなくても、本番走行開始時刻になったらレースは開始されます
    - 注意：運営が用意するAutoware PCの場合、`~/aichallenge-racingkart/aichallenge/workspace/` には最新のmainブランチでビルドした`build`フォルダと`install`フォルダが格納されています。これは、ビルド時間を短縮するための措置です。気になる場合には削除してクリーンビルドを行ってください。
