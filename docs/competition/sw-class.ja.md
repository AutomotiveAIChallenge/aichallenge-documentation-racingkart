...
| Sim to Real部門 SIM決勝準備 | 9月18日 | SIM決勝当日と同じ環境で動作確認・練習走行を行います | SIM予選の上位32チーム |
...
- コースの壁や他車両への衝突等が発生した場合・不正な加速度が入力された場合、ペナルティとして一定時間速度が制限されます。（SIM環境のみ）
...
- 走行は3台同時のレース方式で行われます。周回数は6周で、完走順に順位が決まります。
...
- 走行は4台同時のレース方式で行われます。周回数は6周で、完走順に順位が決まります。
- 各クラス4試合行い、各試合の上位2チームが9月20日の実機決勝に進出となります。順位はベストラップによって決まります。
...
- クラッシュ等で車両がスタックしたり、車両が予期せぬ挙動をした場合
    - 運営側のサポートによる復帰は行いません
    - 参加者は自身のターミナル・RVizから任意の復帰操作が可能です。
        - 想定する復帰操作は、Autowareの再起動、自己位置の再設定、手動運転による車両位置の移動です。
        - ただし、操作前には運営に申告する必要があります。また、手動運転を多用することは禁止です。
...
- 走行開始位置はSIM決勝の順位によって決まります。ただし、安全を考慮して走行間隔は予選よりも長くなります。
- クラス別のトーナメント戦の後、各クラスの上位チームによるクラス混合戦を行います。

### 反則の定義 { #final-foul }

以下のような走行は、意図的か偶発的か、または不具合によるものかを問わず反則となります。
...
- 基本的なルールは上述の共通ルールに従います。
- クラッシュ等で車両がスタックしたり、車両が予期せぬ挙動をした場合
    - 運営側のサポートによる復帰は行いません（遠隔操作、物理的介入どちらも行いません）
