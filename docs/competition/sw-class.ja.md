...
- コースの壁や他車両への衝突等が発生した場合や、不正な加速度が入力された場合、ペナルティとして一定時間速度が制限されます。（SIM環境のみ）
...
- SIM決勝としての順位は、実機決勝に進出する上位チームの中で以下の順で決定します。本順位は実機決勝の走行位置を決めるために使われます。（SIM決勝順位による表彰はありません）
...
- 試合後10分間で、試合の振り返りを行います。解説員からの質問にご回答をお願いします。
