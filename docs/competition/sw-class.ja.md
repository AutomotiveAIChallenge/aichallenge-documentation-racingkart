...
後退（バック）中の車両が後方の車両に衝突した場合は、**後退した側（前方の車両）にも `CRASH` が科されます**。自車の後バンパーと他車の前バンパーの接触で発火するため、後退した車両と衝突された後方の車両の双方がペナルティを受けます。コース復帰などでバックする際は、後方の車両が通過するのを待ってから動き出してください。
...
以下の安全ゲートをすべてクリアすることを強く推奨します。必須ではありませんが、複数台での走行で勝つためには、これらの安全ゲートをクリアして走行できていることが重要です。実行方法は[こちら](../development/development-guide.ja.md#safety-gate)をご確認ください。
...
- V2X で得た後方車両の情報を使って、後方車両の進路を妨害すること。幅寄せ・蛇行・不要な減速を含みます。リバースでのコース復帰時など、安全確認のために後方情報を参照することは禁止しません。
...
- 1日に提出できるコードの回数には上限があります（2026年8月時点で1日10回・提出間隔30分）。
...
- SIM決勝の詳細は[SIM決勝特設ページ](./sim-finals.ja.md)をご確認ください。
...
- 走行は4台同時のレース方式で行われます。周回数は6周で、各レースでの順位は完走順で決まります。
    - ただし、完走する前にタイムアウトになった場合は周回数+セクション数が多いほど上位になります。
    - それでも同順位の場合は、ペナルティ回数が少ないほど上位になります。
    - それでも同順位の場合は、SIM予選での順位で決まります。
...
- SIM決勝としての順位は、各試合の着順ごとにグループ分けして決定します。各試合1位の4チームが1〜4位、各試合2位の4チームが5〜8位、各試合3位の4チームが9〜12位、各試合4位の4チームが13〜16位となり、各グループ内では以下の順で決定します。
...
- 参加者は自身のターミナル・RVizから任意の操作が可能です（ただしAWSIM画面は操作できません）。想定される操作は以下のとおりです。
    ...
    - 自己位置の再設定
    - スタック時に、手動運転による車両の移動
        - AWSIMによる手動モードは使用出来ないため、各自で組み込む必要があります
        - 手動運転を多用することは禁止です。正常位置に復帰後は手動運転を直ちに解除する必要があります
    - `ros2 topic` コマンドによるギア切り替え・ターボコマンド発行
- ただし、操作前には運営に申告する必要があります。自身のROS_DOMAIN_ID以外に影響を及ぼす操作も禁止です。
...
- 規定時間（7分）の間、車両は走り続けます。周回数の上限はありません。
- 順位は以下の順で決定します。
    1. 反則回数が少ないほど上位
    2. 反則回数が同数の場合は、周回数+セクション数が多いほど上位
        - 例えば、2周完走+3周目の第2セクションまで走行したチームと、2周+3周目して第3セクションまで走行したチームなら、後者が上位になります
...
- 基本的なルールは上述の共通ルールに従います。
- 「自動走行」と「ステアのみ自動」モードのみが許可されます。「ステアのみ自動」モードではアクセルのみゲームコントローラで手動操作します。
- ゲームコントローラによる「手動走行」は以下の条件で使用可能です。
    - 3回につき1回分の反則が加算されます。
    - コース上の正常位置に戻ったら、5秒以内に手動走行を解除する必要があります。
    - 「手動走行」に切り替える前には運営に申告する必要があります。
- クラッシュ等で車両がスタックしたり、車両が予期せぬ挙動をした場合、運営側のサポートによる復帰は行いません（遠隔操作、物理的介入どちらも行いません）。
    - ただし、運営が危険と判断した場合に緊急停止・車両移動を行う可能性はあります。
- 参加者は自身のターミナル・RVizから任意の操作が可能です。ただし、操作前には運営に申告する必要があります。自身のROS_DOMAIN_ID以外に影響を及ぼす操作も禁止です。
...
- 全車両が走行不能など、試合不成立の場合には再試合が行われることがあります。スタート位置から再度やり直しとなり、反則回数や周回数はリセットされます。再試合の判断は運営が行います。
- 運営は試合開始前に、Driver、通信、Autoware起動が問題ないことを確認した上で参加者に引き渡します。試合中のトラブルに関しては運営としては責任を持ちません。ただし、通信トラブルなどで全車両走行が不可能になった場合は、再走行といった措置を取る可能性があります
