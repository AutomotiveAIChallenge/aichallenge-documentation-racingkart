...
#### 追い越しレーン { #overtaking-lane }

SIM決勝では、後方車両の進路を塞ぐ走行を抑えるため、コース上の指定区間を**追い越しレーン**とします。判定は AWSIM が自動で行います。（SIM決勝のみ。End to End部門には適用しません）

- 追い越しレーンは**水色の長方形**として路面に描画されます。
- 車体全体が追い越しレーン内に入り、**27 km/h 以上**で走行する車両を**アタッカー**とします。アタッカーがいる間、追い越しレーンはオレンジ色に脈動します。
- アタッカーがいる間、追い越しレーンに触れている他の車両は、**27 km/h 以下**なら**3秒以内**に完全に退出しなければ違反です。加速しても退出の義務は消えません。
- アタッカーがいる間、追い越しレーン外の車両が **27 km/h 未満**で追い越しレーンに触れた場合も違反です。
- 違反した車両には `BLOCK` ペナルティ（**20秒間・5 km/h の速度制限**）が科されます。HUD に `BLOCK` と残り秒数が表示されます。

![overtaking_lane_normal](../assets/overtaking_lane_normal.png)

通常時の状態です。路面が水色になっている範囲が追い越しレーンです。

![overtaking_lane_active](../assets/overtaking_lane_active.png)

追い越しレーン内に 27 km/h 以上の車両がいる間は、オレンジ色に変わります。

追い越しレーンに進入しなければ制約はありません。練習方法は[シミュレータ仕様](../specifications/simulator.ja.md#launch-modes)を参照してください。
