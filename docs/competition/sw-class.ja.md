...
- 加速度上限は約1.0m/s²に制限されています。速度上限はシミュレータが使用している物理モデルに従います。
- レース内順位によって加速度・速度に対してハンディキャップがつく場合があります。
- コースの壁や他車両への衝突等が発生した場合・不正な加速度が入力された場合、ペナルティとして一定時間速度が制限されます。
- 壁に衝突した場合は、自動的に姿勢補正が行われます。ただし、壁に衝突して動けなくなることを100%保証するものではありません。
- 一時的に加速度を上昇させるブーストアイテムが用意されています。
...
- 決勝戦会場で運営が用意した PC 上で、リアルタイム同時対戦を行います。
...
- シティサーキット東京ベイ（CCTB）での実車両を用いて、リアルタイム同時対戦を行います。
- カートに搭載されたPC 上で参加チームのコードを動かします。作業に必要なPC は運営が用意します。
- 安全のため最大車速は30km/hに制限されています
- クラッシュ等で車両がスタックした場合には、手動操作による復帰が許されます。（未定）
- 減速ペナルティや順位によるハンディキャップ、ニトロなどの加速アイテムはありません。（未定）

### 注意事項

- 参加チームは実車両へのインテグレーション作業を行い、決勝当日に走行に立ち会う必要があります。（作業は運営がサポートします）
