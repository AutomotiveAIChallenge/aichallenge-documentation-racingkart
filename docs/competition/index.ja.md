...
| 最終ステージ | 実機決勝 | SIM 決勝 |
| 競技走行環境 | オンライン・運営が用意した PC（SIM 決勝は持参 PC も可） | 参加チーム自身の PC |
| GPU | 使用不可 | 使用可 |
| 参加 | 全員必須 | 希望者のみ |
...
| --- | --- |
| 5月25日 〜 7月31日 | 参加登録期間 |
| 7月1日 〜 9月1日 | 予選期間（Sim to Real部門：オンライン採点環境で走行／E2E AI部門：資料提出） |
| 9月18日 | SIM 決勝準備（オンサイト・東京国際交流館） |
...
!!! warning "E2E AI部門は実車両決勝なし"
    E2E AI部門の最終ステージは SIM 決勝です。実車両決勝は Sim to Real SW 部門のみ実施します。
...
- **SIM 予選 → SIM 決勝 → 実機決勝** の 3 ステップで腕を競います。
    ...
    - :material-monitor: 競技走行は運営が用意した PC で実行するため、参加チームの PC・GPU は不要です（SIM 決勝では持参 PC の使用も可能です）。ただし、ローカルでの開発時に AWSIM を動かすには GPU が必要です。

## End to End AI 部門概要

- 機械学習アルゴリズムによってソフトウェアを改善する部門です。
- カメラ・2D LiDARといった外界センサデータを入力とした単一AIモデルを実装することが期待されます。
- 最終ステージは9月19日の **SIM 決勝** で、実機決勝はありません。
    - :material-account: クラス分けなし（統一クラス）
    - :material-check-circle: Sim to Real SW 部門参加チームが希望制で追加参加可能
