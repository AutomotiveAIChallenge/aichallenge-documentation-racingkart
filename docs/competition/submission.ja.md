...
3. オンライン採点環境への提出
    ...
    !!! note "提出時の制限（2026年8月時点の提出画面の表示）"
        - アップロードできるファイルは **20 MB** までです。学習済みモデルの重みなど大きなファイルを含める場合は注意してください。
        - 次の提出までは **30分** 空ける必要があります（提出ボタンが `Wait MM:SS` 表示になります）。1日の提出回数は **10回** までです（`TODAY'S USAGE n / 10`）。
        - 対戦相手の範囲は自チームより **+1〜+5 位**の間で指定します。範囲内の対戦相手はランダムに決まり、提出ボタンを押した時点のランキングで決定されます。
